...
describe('auth callback urls', () => {
  ...
  test('keeps public dashboard destinations when closing the overlay', () => {
    expect(getLoginCloseUrl('/dashboards/statistics?range=3d')).toBe('/dashboards/statistics?range=3d');
    expect(getLoginCloseUrl('/dashboard/statistics?range=3d')).toBe('/dashboards/statistics?range=3d');
  });
});
