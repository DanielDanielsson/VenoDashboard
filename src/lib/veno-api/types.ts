export interface VenoApiReading {
  id?: string;
  timestamp: string;
  valueMmolL: number;
  valueMgDl: number;
...
export interface VenoApiSourceStatus {
  stable: boolean;
  connected: boolean;
  latestReading: VenoApiReading | null;
  sourceToDbLagMinutes: number | null;
  latestReadingAgeMinutes: number | null;
  syncLagMinutes?: number | null;
}

export interface VenoApiStatusReport {
  generatedAt: string;
  official: VenoApiSourceStatus;
  share: VenoApiSourceStatus;
  tandem: VenoApiSourceStatus;
}
...
export interface VenoApiErrorResponse {
  error?: {
    code?: string;
    message?: string;
  };
