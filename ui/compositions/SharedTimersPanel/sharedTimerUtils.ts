...
export function getServerOffsetMs(serverNow: string): number {
  return new Date(serverNow).getTime() - Date.now();
}

export function sortTimers(items: SharedTimer[]): SharedTimer[] {
...
export function upsertTimer(items: SharedTimer[], timer: SharedTimer): SharedTimer[] {
